...
!
  ...
  INTEGER,  PARAMETER   :: MHC        = 13 !11 ! max # HCs   !(mrq, 05/20) from 11 to 13
  INTEGER,  PARAMETER   :: MSV        = 6  !5  ! max # lumped semivols     !(mrq, 01/22) from 5 to 6
  INTEGER,  PARAMETER   :: MPROD      = 5  !4  ! max # volatility products !(mrq, 02/20) from 4 to 5
  ...
  !(mrq, 05/20)
  INTEGER,  PARAMETER   :: PARENTIVOC5  = 12 ! IVOC surrogate
  INTEGER,  PARAMETER   :: PARENTIVOC4  = 13 ! IVOC surrogate
...
!
  ...
  INTEGER :: id_ASOA3,   id_AW1,    id_BCPI,   id_BCPO     ! remove id_ASOAN  (mrq, 01/22)
  ...
  INTEGER :: id_POA3,    id_POA4,   id_POA5                !(mrq, 01/20)
  INTEGER :: id_POG3,    id_POG4                           !(mrq, 06/20)
  INTEGER :: id_OPOA3                                      !(mrq, 06/20)
  INTEGER :: id_ASOG0,   id_ASOA0                          !(mrq, 02/20)
  INTEGER :: id_LNRO2N5, id_LNRO2H5,id_LNRO2N4,id_LNRO2H4  !(mrq, 05/20)
  INTEGER :: id_IVOC5,   id_IVOC4                          !(mrq, 05/20)
  INTEGER :: id_IVSOA0,  id_IVSOA1, id_IVSOA2,  id_IVSOA3  !(mrq, 01/22)
  INTEGER :: id_IVSOG0,  id_IVSOG1, id_IVSOG2,  id_IVSOG3  !(mrq, 01/22)
  INTEGER :: id_IVSOAN                                     !(mrq, 01/22)
  INTEGER :: id_ISOG1,   id_ISOG2,  id_ISOG3,  id_ISOG0    !(mrq, 01/22)
  INTEGER :: id_ISOA1,   id_ISOA2,  id_ISOA3,  id_ISOA0    !(mrq, 01/22)
...
!
    ...
    IF ( FIRSTCHEM ) THEN
       ...
       IF ( LSOA ) THEN
          ...
          !(mrq, 01/22) from 3 to 4
          MAXSIMSV = 4 !3  ! mono+sesq (1) + isop (2) + aromatics (3) + IVOC (4)
...
!
   ...
   USE State_Diag_Mod, ONLY : DgnState
   USE State_Grid_Mod, ONLY : GrdState
   USE TIME_MOD,       ONLY : GET_TS_CHEM  !(mrq, 07/22)
#ifdef APM
   USE APM_INIT_MOD,   ONLY : APMIDS
...
!
   ...
   REAL(fp)        :: DTCHEM  !(mrq, 07/22)

   ! Parent HC reacted diag for arom/IVOC (hotp 5/17/10)
   ! (mrq, 06/20) add IVOC5 and IVOC4 (from 8 to 12)
   REAL(fp)        :: DARO2_TOT_0(12)  !DARO2_TOT_0(8)             ! added NAP (hotp 7/22/09)
   REAL(fp), SAVE  :: DARO2_TOT(12) = 0e+0_fp  !DARO2_TOT(8) = 0e+0_fp     ! added NAP (hotp 7/22/09)
   ...
   ! Chemistry timestep [s] !(mrq, 07/22)
   DTCHEM  = GET_TS_CHEM()
   ...
   IF ( FIRST ) THEN
      ...
      IF ( prtDebug ) THEN
         ...
         print*, 'IVOC5', PARENTIVOC5,IDSV(PARENTIVOC5)  !(mrq, 05/20)
         print*, 'IVOC4', PARENTIVOC4,IDSV(PARENTIVOC4)  !(mrq, 05/20)
         print*, 'POA and OPOA'
      ...
      DELTAHCSAVE    = 0e+0_fp
      !SPECSOAPROD    = 0e+0_fp !(mrq, 07/22)
      !SPECSOAEVAP    = 0e+0_fp !(mrq, 07/22)
...
!          print*,'POA3', SUM(Spc(:,:,:,id_POA3)) !(mrq, 01/20)
!          print*,'POA4', SUM(Spc(:,:,:,id_POA4)) !(mrq, 01/20)
!          print*,'POA5', SUM(Spc(:,:,:,id_POA5)) !(mrq, 01/20)
!          print*,'POG1', SUM(Spc(:,:,:,id_POG1))
!          print*,'POG2', SUM(Spc(:,:,:,id_POG2))
!          print*,'POG3', SUM(Spc(:,:,:,id_POG3)) !(mrq, 01/20)
!          print*,'POG4', SUM(Spc(:,:,:,id_POG4)) !(mrq, 01/20)
!          print*,'OPOA1',SUM(Spc(:,:,:,id_OPOA1))
!          print*,'OPOA2',SUM(Spc(:,:,:,id_OPOA2))
!          print*,'OPOA3',SUM(Spc(:,:,:,id_OPOA3)) !(mrq, 01/20)
...
!       print*, 'POAEMISS,3,POG3', &
!                SUM(POAEMISS(:,:,:,3)),POAEMISS(IIDEBUG,JJDEBUG,2,3), &!(mrq, 01/20)
!                   POAEMISS(IIDEBUG,JJDEBUG,10,3)
!       print*, 'POAEMISS,4,POG4', &
!                SUM(POAEMISS(:,:,:,4)),POAEMISS(IIDEBUG,JJDEBUG,2,4), &!(mrq, 01/20)
!                    POAEMISS(IIDEBUG,JJDEBUG,10,4)
!       print*, 'POAEMISS,5,POA5', &
!                SUM(POAEMISS(:,:,:,5)),POAEMISS(IIDEBUG,JJDEBUG,2,5), &!(mrq, 01/20)
!                    POAEMISS(IIDEBUG,JJDEBUG,10,5)

!       ! 10/12/09 debug
!       IF ( Input_Opt%LSVPOA ) THEN
!          print*,'POAG tot', SUM(Spc(:,:,:,id_POA1))+ &
!                             SUM(Spc(:,:,:,id_POA2))+ &
!                             SUM(Spc(:,:,:,id_POA3))+ &  !(mrq, 01/22)
!                             SUM(Spc(:,:,:,id_POA4))+ &  !(mrq, 01/22)
!                             SUM(Spc(:,:,:,id_POA5))+ &  !(mrq, 01/22)
!                             SUM(Spc(:,:,:,id_POG1))+ &
!                             SUM(Spc(:,:,:,id_POG2))+ &
!                             SUM(Spc(:,:,:,id_POG3))+ &  !(mrq, 01/22)
!                             SUM(Spc(:,:,:,id_POG4))     !(mrq, 01/22)
!       ENDIF
!    ENDIF
   ...
   DO I = 1, State_Grid%NX
      ...
      ! (mrq, 01/22) remove nonvolatile ASOA 
      ...
      ! (mrq, 06/20) enforce direct yield for POA5 and OPOA3
      !POA5
      JHC = PARENTPOA
      JSV = IDSV(JHC)
      IPR = 5 
    ...
    !(mrq, 07/22)
    !--------------------------------------------
    IF ( State_Diag%Archive_ProdtotSOGfromTerpinHighNOx ) THEN
        NOX = NHIGHNOX
        State_Diag%ProdtotSOGfromTerpinHighNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTMTPA)*sum(ALPHA(NOX,:,PARENTMTPA)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTLIMO)*sum(ALPHA(NOX,:,PARENTLIMO)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTMTPO)*sum(ALPHA(NOX,:,PARENTMTPO)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTSESQ)*sum(ALPHA(NOX,:,PARENTSESQ)) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotSOGfromTerpinLowNOx ) THEN
        NOX = NLOWNOX
        State_Diag%ProdtotSOGfromTerpinLowNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTMTPA)*sum(ALPHA(NOX,:,PARENTMTPA)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTLIMO)*sum(ALPHA(NOX,:,PARENTLIMO)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTMTPO)*sum(ALPHA(NOX,:,PARENTMTPO)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTSESQ)*sum(ALPHA(NOX,:,PARENTSESQ)) )/ DTCHEM
    ...
    IF ( State_Diag%Archive_ProdtotSOGfromTerpandNO3 ) THEN
        NOX = NNO3RXN
        State_Diag%ProdtotSOGfromTerpandNO3(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTMTPA)*sum(ALPHA(NOX,:,PARENTMTPA)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTLIMO)*sum(ALPHA(NOX,:,PARENTLIMO)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTMTPO)*sum(ALPHA(NOX,:,PARENTMTPO)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTSESQ)*sum(ALPHA(NOX,:,PARENTSESQ)) )/ DTCHEM
    ...
    IF ( State_Diag%Archive_ProdtotSOGfromIsopinHighNOx ) THEN
        NOX = NHIGHNOX
        State_Diag%ProdtotSOGfromIsopinHighNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTISOP)*sum(ALPHA(NOX,:,PARENTISOP)) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotSOGfromAVOCsinHighNOx ) THEN
        NOX = NHIGHNOX
        State_Diag%ProdtotSOGfromAVOCsinHighNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTBENZ)*sum(ALPHA(NOX,:,PARENTBENZ)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTTOLU)*sum(ALPHA(NOX,:,PARENTTOLU)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTXYLE)*sum(ALPHA(NOX,:,PARENTXYLE)) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotSOGfromAVOCsinLowNOx ) THEN
        NOX = NLOWNOX
        State_Diag%ProdtotSOGfromAVOCsinLowNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTBENZ)*sum(ALPHA(NOX,:,PARENTBENZ)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTTOLU)*sum(ALPHA(NOX,:,PARENTTOLU)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTXYLE)*sum(ALPHA(NOX,:,PARENTXYLE)) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotSOGfromIVOCsinHighNOx ) THEN
        NOX = NHIGHNOX
        State_Diag%ProdtotSOGfromIVOCsinHighNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTNAP)*sum(ALPHA(NOX,:,PARENTNAP)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTIVOC5)*sum(ALPHA(NOX,:,PARENTIVOC5)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTIVOC4)*sum(ALPHA(NOX,:,PARENTIVOC4)) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotSOGfromIVOCsinLowNOx ) THEN
        NOX = NLOWNOX
        State_Diag%ProdtotSOGfromIVOCsinLowNOx(I,J,L) = &
            ( DELTASOGSAVE(I,J,L,NOX,PARENTNAP)*sum(ALPHA(NOX,:,PARENTNAP)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTIVOC5)*sum(ALPHA(NOX,:,PARENTIVOC5)) &
          + DELTASOGSAVE(I,J,L,NOX,PARENTIVOC4)*sum(ALPHA(NOX,:,PARENTIVOC4)) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotSOGfromSVOCs ) THEN
        State_Diag%ProdtotSOGfromSVOCs(I,J,L) = &
            sum( DELTASOGSAVE(I,J,L,:,PARENTOPOA) )/ DTCHEM
    ENDIF

    IF ( State_Diag%Archive_ProdtotPOGfromPOGemit ) THEN
        State_Diag%ProdtotPOGfromPOGemit(I,J,L) = &
             DELTASOGSAVE(I,J,L, 1,PARENTPOA) / DTCHEM
    ENDIF
...
!                                MAXVAL(GLOB_DARO2(:,:,:,1,1))
      ...
      !(mrq, 01/22) added IVOC5 and IVOC4 diagnostic info
      DARO2_TOT(9) = DARO2_TOT(9) + SUM(GLOB_DARO2(:,:,:,1,5)) / 1d9
      DARO2_TOT(10) = DARO2_TOT(10) + SUM(GLOB_DARO2(:,:,:,2,5)) / 1d9
      DARO2_TOT(11) = DARO2_TOT(11) + SUM(GLOB_DARO2(:,:,:,1,6)) / 1d9
      DARO2_TOT(12) = DARO2_TOT(12) + SUM(GLOB_DARO2(:,:,:,2,6)) / 1d9
...
!       print*,'Units are Tg of parent'
      ...
      !(mrq, 01/22) added IVOC5 and IVOC4 diagnostic info
      print*, 'GL_DAR NOX IVOC5 =', DARO2_TOT(9), (DARO2_TOT(9) - DARO2_TOT_0(9))
      print*, 'GL_DAR HO2 IVOC5 =', DARO2_TOT(10), (DARO2_TOT(10) - DARO2_TOT_0(10))
      print*, 'GL_DAR NOX IVOC4 =', DARO2_TOT(11), (DARO2_TOT(11) - DARO2_TOT_0(11))
      print*, 'GL_DAR HO2 IVOC4 =', DARO2_TOT(12), (DARO2_TOT(12) - DARO2_TOT_0(12))
...
!          print*,'POA1 ', SUM(Spc(:,:,:,id_POA1))
!          print*,'POA2 ', SUM(Spc(:,:,:,id_POA2))
!          print*,'POA3 ', SUM(Spc(:,:,:,id_POA3))   !(mrq, 01/22)
!          print*,'POA4 ', SUM(Spc(:,:,:,id_POA4))   !(mrq, 01/22)
!          print*,'POA5 ', SUM(Spc(:,:,:,id_POA5))   !(mrq, 01/22)
!          print*,'POG1 ', SUM(Spc(:,:,:,id_POG1))
!          print*,'POG2 ', SUM(Spc(:,:,:,id_POG2))
!          print*,'POG3 ', SUM(Spc(:,:,:,id_POG3))   !(mrq, 01/22)
!          print*,'POG4 ', SUM(Spc(:,:,:,id_POG4))   !(mrq, 01/22)
!          print*,'OPOA1', SUM(Spc(:,:,:,id_OPOA1))
!          print*,'OPOA2', SUM(Spc(:,:,:,id_OPOA2))
!          print*,'OPOA3', SUM(Spc(:,:,:,id_OPOA3))  !(mrq, 01/22)
!          print*,'OPOG1', SUM(Spc(:,:,:,id_OPOG1))
!          print*,'OPOG2', SUM(Spc(:,:,:,id_OPOG2))
!       ENDIF
...
!       print*, 'POGRXN p3', SUM(GLOB_POGRXN(:,:,:,3))  !(mrq, 01/22)
!       print*, 'POGRXN p4', SUM(GLOB_POGRXN(:,:,:,4))  !(mrq, 01/22)

!       ! semivolpoa3: POG1 + POG2
!       print*, 'POAEMISS tot', SUM(POAEMISS(:,:,:,:)), &
!                POAEMISS(IIDEBUG,JJDEBUG,2,1)+POAEMISS(IIDEBUG,JJDEBUG,2,2)+ &
!                POAEMISS(IIDEBUG,JJDEBUG,2,3)+POAEMISS(IIDEBUG,JJDEBUG,2,4)+ &
!                POAEMISS(IIDEBUG,JJDEBUG,2,5), &
!                POAEMISS(IIDEBUG,JJDEBUG,10,1)+POAEMISS(IIDEBUG,JJDEBUG,10,2)+ &
!                POAEMISS(IIDEBUG,JJDEBUG,10,3)+POAEMISS(IIDEBUG,JJDEBUG,10,4)+ &  !(mrq, 01/20)
!                POAEMISS(IIDEBUG,JJDEBUG,10,5)     !(mrq, 01/20)
...
!
 SUBROUTINE SOA_PARA( TEMP, KO3, KOH, KNO3, KOM, II, JJ, LL, &
                      KRO2NO, KRO2HO2, State_Met, Input_Opt )
!
! !USES:
!
   USE State_Met_Mod,      ONLY : MetState
...
!
   ...
   ! update dims (hotp 5/22/10)
   DO JSV = 1, MAXSIMSV !3   !(mrq, 01/22) move 3 to MAXSIMSV
   DO IPR = 1, NPROD(JSV)
   ...
   !TMP1 = ( TEMP / 298.e+0_fp ) !(mrq, 01/20) refer to 298K

   ! Compute the heat-of-vaporization exponential term outside the DO loop
   !TMP2 = EXP( HEAT_VAPOR * ( OVER - REF300 ) )
   !TMP2 = EXP( HEAT_VAPOR * ( OVER - REF298 ) ) !(mrq, 01/20) refer to 298K

   ! Multiply KOM by the temperature and heat-of-vaporization terms
   ! Adjust POA from reference of 300K
   !(mrq, 01/22) comment out
   !JHC = PARENTPOA
   !JSV = IDSV(JHC)
   !DO IPR = 1, NPROD(JSV)
   !   KOM(IPR,JSV) = KOM_REF(IPR,JSV) * TMP1 * TMP2
...
!
   ...
   ALPHA(NOX,1,PARENTMTPA) = 0.000e+0_fp
   ALPHA(NOX,2,PARENTMTPA) = 0.000e+0_fp
   ALPHA(NOX,3,PARENTMTPA) = 0.531e+0_fp
   ALPHA(NOX,4,PARENTMTPA) = 0.027e+0_fp
   NOX = NLOWNOX
   ! (mrq, 01/22) apply new SOA yield
   ...
   ALPHA(NOX,1,PARENTMTPA) = 0.000e+0_fp
   ALPHA(NOX,2,PARENTMTPA) = 0.077e+0_fp
   ALPHA(NOX,3,PARENTMTPA) = 0.677e+0_fp
   ALPHA(NOX,4,PARENTMTPA) = 0.139e+0_fp
   NOX = NNO3RXN
   ...
   ALPHA(NOX,1,PARENTSESQ) = 0.000e+0_fp
   ALPHA(NOX,2,PARENTSESQ) = 0.723e+0_fp
   ALPHA(NOX,3,PARENTSESQ) = 0.725e+0_fp
   ALPHA(NOX,4,PARENTSESQ) = 0.000e+0_fp
   NOX = NLOWNOX
   ! (mrq, 01/22) apply new SOA yield
   ...
   ALPHA(NOX,1,PARENTSESQ) = 0.000e+0_fp
   ALPHA(NOX,2,PARENTSESQ) = 0.373e+0_fp
   ALPHA(NOX,3,PARENTSESQ) = 0.661e+0_fp
   ALPHA(NOX,4,PARENTSESQ) = 0.079e+0_fp
   NOX = NNO3RXN
   ALPHA(NOX,1,PARENTSESQ) = 0.0000e+0_fp
   ...
   NOX = NHIGHNOX
   ALPHA(NOX,1,PARENTISOP) = 0.000e+0_fp * 2.2
   ALPHA(NOX,2,PARENTISOP) = 0.000e+0_fp * 2.2
   ALPHA(NOX,3,PARENTISOP) = 0.017e+0_fp * 2.2 
   ALPHA(NOX,4,PARENTISOP) = 0.000e+0_fp * 2.2
   !(mrq, 07/22) no SOA formed from low NOx and NO3 rxn
   NOX = NLOWNOX
   ALPHA(NOX,1,PARENTISOP) = 0.000e+0_fp !0.016e+0_fp
   ALPHA(NOX,2,PARENTISOP) = 0.000e+0_fp !0.012e+0_fp
   ALPHA(NOX,3,PARENTISOP) = 0.000e+0_fp !0.160e+0_fp
   ALPHA(NOX,4,PARENTISOP) = 0.000e+0_fp !0.006e+0_fp
   !------------------------------------
   NOX = 2 ! NO3 rxn
   ALPHA(NOX,1,PARENTISOP) = 0.000e+0_fp !0.0000e+0_fp
   ALPHA(NOX,2,PARENTISOP) = 0.000e+0_fp !0.2171e+0_fp
   ALPHA(NOX,3,PARENTISOP) = 0.000e+0_fp !0.0919e+0_fp
   ...
   NOX = NHIGHNOX
   ALPHA(NOX,1,PARENTBENZ) = 0.024e+0_fp * 1.2
   ALPHA(NOX,2,PARENTBENZ) = 0.000e+0_fp * 1.2
   ALPHA(NOX,3,PARENTBENZ) = 0.835e+0_fp * 1.2
   ALPHA(NOX,4,PARENTBENZ) = 0.048e+0_fp * 1.2
   ! LOW NOX BENZ (non-volatile)
   !ALPHA(2,4,PARENTBENZ) = 0.37e+0_fp
   NOX = NLOWNOX
   ALPHA(NOX,1,PARENTBENZ) = 0.000e+0_fp * 1.9
   ALPHA(NOX,2,PARENTBENZ) = 0.000e+0_fp * 1.9
   ALPHA(NOX,3,PARENTBENZ) = 0.485e+0_fp * 1.9
   ALPHA(NOX,4,PARENTBENZ) = 0.000e+0_fp * 1.9
   ...
   NOX = NHIGHNOX
   ALPHA(NOX,1,PARENTTOLU) = 0.000e+0_fp * 1.2
   ALPHA(NOX,2,PARENTTOLU) = 0.105e+0_fp * 1.2
   ALPHA(NOX,3,PARENTTOLU) = 0.084e+0_fp * 1.2
   ALPHA(NOX,4,PARENTTOLU) = 0.023e+0_fp * 1.2
   ! LOW NOX TOLU
   !ALPHA(2,4,PARENTTOLU) = 0.30e+0_fp
   NOX = NLOWNOX
   ALPHA(NOX,1,PARENTTOLU) = 0.000e+0_fp * 1.9
   ALPHA(NOX,2,PARENTTOLU) = 0.000e+0_fp * 1.9
   ALPHA(NOX,3,PARENTTOLU) = 0.959e+0_fp * 1.9
   ALPHA(NOX,4,PARENTTOLU) = 0.000e+0_fp * 1.9
   ...
   NOX = NHIGHNOX
   ALPHA(NOX,1,PARENTXYLE) = 0.000e+0_fp * 1.2
   ALPHA(NOX,2,PARENTXYLE) = 0.049e+0_fp * 1.2
   ALPHA(NOX,3,PARENTXYLE) = 0.076e+0_fp * 1.2
   ALPHA(NOX,4,PARENTXYLE) = 0.016e+0_fp * 1.2
   ! LOW NOX XYLE
   !ALPHA(2,4,PARENTXYLE) = 0.36e+0_fp
   NOX = NLOWNOX
   ALPHA(NOX,1,PARENTXYLE) = 0.000e+0_fp * 1.9
   ALPHA(NOX,2,PARENTXYLE) = 0.000e+0_fp * 1.9
   ALPHA(NOX,3,PARENTXYLE) = 0.549e+0_fp * 1.9
   ALPHA(NOX,4,PARENTXYLE) = 0.006e+0_fp * 1.9
   ...
   !ALPHA(1,1,PARENTPOA) = 0.49e+0_fp  !(mrq, 02/20)
   !ALPHA(1,2,PARENTPOA) = 0.51e+0_fp  !(mrq, 02/20)
   ! No high NOx parameters
   ! semivolpoa3: add diesel/anthropogenic POA (hotp 3/13/09)
   !ALPHA(2:MNOX,1:MPROD,10) = 0e+0_fp
   ALPHA(1,1,PARENTPOA) = 1.e+0_fp  !(mrq, 02/20)
   ALPHA(1,2,PARENTPOA) = 1.e+0_fp  !(mrq, 02/20)
   ALPHA(1,3,PARENTPOA) = 1.e+0_fp  !(mrq, 02/20)
   ALPHA(1,4,PARENTPOA) = 1.e+0_fp  !(mrq, 02/20)
   ALPHA(1,5,PARENTPOA) = 1.e+0_fp  !(mrq, 02/20)
   ...
   ALPHA(1,3,PARENTOPOA) = 1.e+0_fp  !(mrq, 02/20)
   ! anthropogenic
   ...
   ! HIGH NOX
   ! Ox = NO
   !(mrq, 02/20) modify the SOA yield of IVOCs
   !(mrq, 09/22) correct wall loss
   !ALPHA(1,1,PARENTNAP) = 0.0387e+0_fp
   !ALPHA(1,2,PARENTNAP) = 0.2956e+0_fp
   !ALPHA(1,3,PARENTNAP) = 0.2349e+0_fp
   ALPHA(1,1,PARENTNAP) = 0.052e+0_fp * 1.2
   ALPHA(1,2,PARENTNAP) = 0.201e+0_fp * 1.2
   ALPHA(1,3,PARENTNAP) = 0.296e+0_fp * 1.2
   ALPHA(1,4,PARENTNAP) = 0.011e+0_fp * 1.2
   ! LOW NOX
   ! Ox = HO2
   !ALPHA(2,4,PARENTNAP) = 0.73e+0_fp
   ALPHA(2,5,PARENTNAP) = 0.73e+0_fp * 1.2  !(mrq, 02/20) from 4 to 5

   !(mrq, 05/20) add SOA yield of IVOC5 and IVOC4
   ALPHA(1,1,PARENTIVOC5) = 0.078e+0_fp * 1.2
   ALPHA(1,2,PARENTIVOC5) = 0.439e+0_fp * 1.2
   ALPHA(1,3,PARENTIVOC5) = 0.271e+0_fp * 1.2
   ALPHA(1,4,PARENTIVOC5) = 0.049e+0_fp * 1.2
   ! LOW NOX
   ! Ox = HO2
   ALPHA(2,5,PARENTIVOC5) = 0.73e+0_fp * 1.2 !(mrq, 02/20) from 4 to 5

   ALPHA(1,1,PARENTIVOC4) = 0.089e+0_fp * 1.2
   ALPHA(1,2,PARENTIVOC4) = 0.550e+0_fp * 1.2
   ALPHA(1,3,PARENTIVOC4) = 0.200e+0_fp * 1.2
   ALPHA(1,4,PARENTIVOC4) = 0.063e+0_fp * 1.2
   ! LOW NOX
   ! Ox = HO2
   ALPHA(2,5,PARENTIVOC4) = 0.73e+0_fp * 1.2 !(mrq, 02/20) from 4 to 5
   ...
   KOM_REF(4,IDSV(PARENTISOP)) = 1.0e+0_fp/0.1e+0_fp   !(mrq, 01/22)

   !---------------------------------------
   ! SEMIVOLATILE 3: BENZ, TOLU, XYLE   !(mrq, 01/22) move NAP to 4
   !---------------------------------------
   ...
   KOM_REF(4,IDSV(PARENTBENZ)) = 1.0e+0_fp/0.1e+0_fp   !(mrq, 01/22)
   ...
   !-------------------------------------------------------
   !---------------------------------------
   ! SEMIVOLATILE 4: NAP, IVOC5, IVOC4 
   !---------------------------------------
   ! Update aromatics to new fits (hotp 5/12/10)
   ! BENZ, TOLU, XYLE, NAP/IVOC all lumped together
   KOM_REF(1,IDSV(PARENTNAP)) = 1.0e+0_fp/1.0e+0_fp
   KOM_REF(2,IDSV(PARENTNAP)) = 1.0e+0_fp/10.0e+0_fp
   KOM_REF(3,IDSV(PARENTNAP)) = 1.0e+0_fp/100.0e+0_fp
   KOM_REF(4,IDSV(PARENTNAP)) = 1.0e+0_fp/0.1e+0_fp 
   ! Low NOX (HO2) non-volatile
   !KOM_REF(4,IDSV(PARENTBENZ)) = 1.d6
   KOM_REF(5,IDSV(PARENTNAP)) = 1.e+10_fp ! more non-vol (hotp 5/28/10)
   !-------------------------------------------------------

   !---------------------------------------
   ! SEMIVOLATILE 5: POA/SVOCs !(mrq, 01/22) change order from 4 to 5
   !---------------------------------------
   ! semivolpoa2: KOM for POA (hotp 2/27/09)
   ! based on Shrivastava et al. 2006 ES&T
   ! Only 2 products (wood smoke)
   ! Tref is 27 C = 300 K
   !(mrq, 01/20) modify POA volatility distribution 
   !KOM_REF(1,IDSV(PARENTPOA)) = 1e+0_fp/1646e+0_fp
   !KOM_REF(2,IDSV(PARENTPOA)) = 1e+0_fp/20e+0_fp
   KOM_REF(1,IDSV(PARENTPOA)) = 1.0e+0_fp/100.0e+0_fp
   KOM_REF(2,IDSV(PARENTPOA)) = 1.0e+0_fp/10.0e+0_fp
   KOM_REF(3,IDSV(PARENTPOA)) = 1.0e+0_fp/1.0e+0_fp
   KOM_REF(4,IDSV(PARENTPOA)) = 1.0e+0_fp/0.1e+0_fp
   KOM_REF(5,IDSV(PARENTPOA)) = 1.e+10_fp      !(mrq, 06/20) non-vol
   ! No high NOx parameters
   ! remove semivolpoa3 changes (hotp 3/27/09)
   ! semivolpoa3: add diesel/anthropogenic POA (hotp 3/13/09)
...
!
   ...
   IF ( id_POA1 > 0 ) THEN
      !(mrq, 05/20)  set be 13 for IVOC5 and IVOC4
      !MAXLOOP = PARENTNAP   !11
      MAXLOOP = PARENTIVOC4   !13
   ELSE
   ...
   DO JHC = 1, MAXLOOP
      ...
      ELSEIF ( JHC == PARENTISOP ) THEN
         ...
         ! Save diagnostic info for bug check (hotp 5/22/10)
         ! convert from kgC to kg     !(mrq, 01/22) no need for conversion 
         ...
         !------------------------------------------
         ! update dims (hotp 5/22/10)
         !(mrq, 01/22) no need for conversion
         DO NOX = 1, NNOX(JSV)
         DO IPR = 1, NPROD(JSV)
            GM0(IPR,JSV) = GM0(IPR,JSV) + ALPHA(NOX,IPR,JHC) * DELHC(NOX) !&
      ...
      ELSEIF ( JHC == PARENTBENZ .or. JHC == PARENTTOLU .or. &
               JHC == PARENTXYLE   ) THEN  !.or. JHC == PARENTNAP (mrq, 01/22)
      ...
      !(mrq, 01/22)
      !------------------------------------------------------
      ELSEIF ( JHC == PARENTNAP .or. JHC == PARENTIVOC5 .or. &
               JHC == PARENTIVOC4   ) THEN
         ...
         ! Diagnostic/debug info (hotp 5/22/10)
         GLOB_DARO2(I,J,L,1:2,JHC-7) = DELHC(1:2)
      ...
      ELSEIF ( JHC == PARENTOPOA ) THEN
         ...
         DO IPR = 1, 4  !NPROD(JSV)   !(mrq, 06/20) fix to 4
         DO NOX = 1, NNOX(JSV)
            ...
            !(mrq, 06/20) add product from POG4 to OPOG3
            !-----------------------------------------------
            !GM0(IPR,JSV) = GM0(IPR,JSV) + ALPHA(NOX,IPR,JHC) * DOH
            IF ( IPR < 4 ) THEN
                GM0(IPR,JSV) = GM0(IPR,JSV) + ALPHA(NOX,IPR,JHC) * DOH
            ELSE
                GM0(3,JSV) = GM0(3,JSV) + ALPHA(NOX,3,JHC) * DOH
            ENDIF
            !------------------------------------------------
            ...
            IF ( IPR < 4 ) THEN
                DELTASOGSAVE(I,J,L,IPR,JHC) = DOH
            ELSE 
                DELTASOGSAVE(I,J,L,3,JHC) = DELTASOGSAVE(I,J,L,3,JHC) + DOH
            ENDIF  
            !-----------------------------------
   ...
   IF ( id_POA1 > 0 ) THEN
      ...
      DO NOX = 1, NNOX(JSV)   ! update dims (hotp 5/22/10)
         ...
         !DELHC(IPR)   = POAEMISS(I,J,L,IPR)
         
         !GM0(IPR,JSV) = GM0(IPR,JSV) + ALPHA(NOX,IPR,JHC)*DELHC(IPR)
         GM0(IPR,JSV) = GM0(IPR,JSV) + POAEMISS(I,J,L,IPR) !DELHC(IPR)
         !---------------------------------------------
         ...
         DELTASOGSAVE(I,J,L,1,JHC) = sum(POAEMISS(I,J,L,:))
...
!
   ...
   !! SEMIVOLATILE 2: ISOP
   !!---------------------------------------
   !(mrq, 01/22) restore isoprene SOA in VBS
   JHC = PARENTISOP
   JSV = IDSV(JHC)
   !! gas phase
   GM0(1,JSV)=Spc(I,J,L,id_ISOG1) ! C* =   1
   GM0(2,JSV)=Spc(I,J,L,id_ISOG2) ! C* =  10
   GM0(3,JSV)=Spc(I,J,L,id_ISOG3) ! C* = 100
   GM0(4,JSV)=Spc(I,J,L,id_ISOG0) ! C* = 0.1   !(mrq, 01/22)
   !! aerosol phase
   AM0(1,JSV)=Spc(I,J,L,id_ISOA1)
   AM0(2,JSV)=Spc(I,J,L,id_ISOA2)
   AM0(3,JSV)=Spc(I,J,L,id_ISOA3)
   AM0(4,JSV)=Spc(I,J,L,id_ISOA0)   !(mrq, 01/22)
   !-----------------------------------------------------------
   ...
   JSV = IDSV(JHC)
   ! gas phase
   GM0(1,JSV)=Spc(I,J,L,id_ASOG1) ! C* =   1
   GM0(2,JSV)=Spc(I,J,L,id_ASOG2) ! C* =  10
   GM0(3,JSV)=Spc(I,J,L,id_ASOG3) ! C* = 100
   GM0(4,JSV)=Spc(I,J,L,id_ASOG0) ! C* = 0.1   !(mrq, 01/22)
   ! aerosol phase
   AM0(1,JSV)=Spc(I,J,L,id_ASOA1)
   AM0(2,JSV)=Spc(I,J,L,id_ASOA2)
   AM0(3,JSV)=Spc(I,J,L,id_ASOA3)
   !AM0(4,JSV)=Spc(I,J,L,id_ASOAN)
   AM0(4,JSV)=Spc(I,J,L,id_ASOA0)   !(mrq, 01/22)
   ...
   !---------------------------------------
   ! Lumped arom/IVOC/NAP semivolatiles (hotp 5/13/10)
   JHC = PARENTNAP ! IDSV(B)=IDSV(T)=IDSV(X)=IDSV(N)
   JSV = IDSV(JHC)
   ! gas phase
   GM0(1,JSV)=Spc(I,J,L,id_IVSOG1) ! C* =   1
   GM0(2,JSV)=Spc(I,J,L,id_IVSOG2) ! C* =  10
   GM0(3,JSV)=Spc(I,J,L,id_IVSOG3) ! C* = 100
   GM0(4,JSV)=Spc(I,J,L,id_IVSOG0) ! C* = 0.1
   ! aerosol phase
   AM0(1,JSV)=Spc(I,J,L,id_IVSOA1)
   AM0(2,JSV)=Spc(I,J,L,id_IVSOA2)
   AM0(3,JSV)=Spc(I,J,L,id_IVSOA3)
   AM0(4,JSV)=Spc(I,J,L,id_IVSOA0)
   AM0(5,JSV)=Spc(I,J,L,id_IVSOAN)
   !----------------------------------------------------------
   ...
   IF ( id_POA1 > 0 .and. id_POG1 > 0 .and. &
        id_POA2 > 0 .and. id_POG2 > 0 .and. &
        id_POA3 > 0 .and. id_POG3 > 0 .and. &
        id_POA4 > 0 .and. id_POG4 > 0 .and. &
        id_POA5 > 0  ) THEN
      ! gas phase
      GM0(1,JSV) = Spc(I,J,L,id_POG1)
      GM0(2,JSV) = Spc(I,J,L,id_POG2)
      GM0(3,JSV) = Spc(I,J,L,id_POG3)  !(mrq, 01/20)
      GM0(4,JSV) = Spc(I,J,L,id_POG4)  !(mrq, 01/20)
      ! aerosol phase
      AM0(1,JSV) = Spc(I,J,L,id_POA1)
      AM0(2,JSV) = Spc(I,J,L,id_POA2)
      AM0(3,JSV) = Spc(I,J,L,id_POA3)  !(mrq, 01/20)
      AM0(4,JSV) = Spc(I,J,L,id_POA4)  !(mrq, 01/20)
      AM0(5,JSV) = Spc(I,J,L,id_POA5)  !(mrq, 01/20)
   ENDIF

   !---------------------------------------
   ! SEMIVOLATILE 5: OPOA/O-SVOCs !(mrq, 01/22) change order from 5 to 6
   !---------------------------------------
   ...
   IF ( id_OPOA1 > 0 .and. id_OPOG1 > 0 .and. &
        id_OPOA2 > 0 .and. id_OPOG2 > 0 .and. &
        id_OPOA3 > 0 ) THEN
      ! gas phase
      ...
      AM0(3,JSV) = Spc(I,J,L,id_OPOA3)  !(mrq, 01/20)
   ENDIF
...
!
   ...
   !(mrq, 07/22)
   !--------------------------------------
   IF ( State_Diag%Archive_ProdSOAfromTerp ) THEN
         State_Diag%ProdSOAfromTerp(I,J,L) = sum(SPECSOAPROD(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   IF ( State_Diag%Archive_EvapSOAfromTerp ) THEN
         State_Diag%EvapSOAfromTerp(I,J,L) = sum(SPECSOAEVAP(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   !--------------------------------------
   ...
   !! (hotp 6/5/10)
   !!-----------------------------
   !(mrq, 01/22) restore isoprene SOA
   id_SPECIES = id_ISOA1
   IPR = 1
   AERCHANGE = AM0(IPR,JSV) - Spc(I,J,L,id_SPECIES)
   IF ( AERCHANGE .GT. 0e+0_fp ) THEN
   ...
   Spc(I,J,L,id_ISOG0) = MAX( GM0(4,JSV), 1e-32_fp ) !(mrq, 01/22)
   ! aerosol phase
   ...
   Spc(I,J,L,id_ISOA0) = MAX( AM0(4,JSV), 1e-32_fp ) !(mrq, 01/22)
   !---------------------------------------------------------------------------

   !(mrq, 07/22)
   !--------------------------------------
   IF ( State_Diag%Archive_ProdSOAfromIsop ) THEN
         State_Diag%ProdSOAfromIsop(I,J,L) = sum(SPECSOAPROD(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   IF ( State_Diag%Archive_EvapSOAfromIsop ) THEN
         State_Diag%EvapSOAfromIsop(I,J,L) = sum(SPECSOAEVAP(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   !--------------------------------------

   !=================================================================
   ! Semivolatile Group 3: benzene, toluene, xylene !(mrq, 01/22), naphthalene/IVOC
   ...
   Spc(I,J,L,id_ASOG0) = MAX( GM0(4,JSV), 1e-32_fp )  !(mrq, 01/22)
   ! aerosol phase
   ...
   Spc(I,J,L,id_ASOA0) = MAX( AM0(4,JSV), 1e-32_fp )  !(mrq, 01/22)
   ! LOW NOX (only 1 aerosol phase)
   !NOX = NLOWNOX ! store in spot 4 (hotp 5/22/10)
   !(mrq, 01/22) remove ASOAN
   !Spc(I,J,L,id_ASOAN) = MAX( AM0(4,JSV), 1e-32_fp )

   !(mrq, 07/22)
   !--------------------------------------
   IF ( State_Diag%Archive_ProdSOAfromAVOCs ) THEN
         State_Diag%ProdSOAfromAVOCs(I,J,L) = sum(SPECSOAPROD(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   IF ( State_Diag%Archive_EvapSOAfromAVOCs ) THEN
         State_Diag%EvapSOAfromAVOCs(I,J,L) = sum(SPECSOAEVAP(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   !--------------------------------------
   ...
   !----------------------------------------------------------
   !=================================================================
   ! Semivolatile Group 3: naphthalene/IVOC  !benzene, toluene, xylene, 
   ! Lump of products of 7-9 Hydrocarbon class (aromatics) (dkh, 11/11/06)
   ! Lumped aromatic/IVOC (hotp 5/13/10
   !=================================================================
   ...
   !NOX = NHIGHNOX
   ! gas phase
   Spc(I,J,L,id_IVSOG1) = MAX( GM0(1,JSV), 1e-32_fp )
   Spc(I,J,L,id_IVSOG2) = MAX( GM0(2,JSV), 1e-32_fp )
   Spc(I,J,L,id_IVSOG3) = MAX( GM0(3,JSV), 1e-32_fp )
   Spc(I,J,L,id_IVSOG0) = MAX( GM0(4,JSV), 1e-32_fp )  !(mrq, 01/22)
   ! aerosol phase
   Spc(I,J,L,id_IVSOA1) = MAX( AM0(1,JSV), 1e-32_fp )
   Spc(I,J,L,id_IVSOA2) = MAX( AM0(2,JSV), 1e-32_fp )
   Spc(I,J,L,id_IVSOA3) = MAX( AM0(3,JSV), 1e-32_fp )
   Spc(I,J,L,id_IVSOA0) = MAX( AM0(4,JSV), 1e-32_fp )  !(mrq, 01/22)
   ! LOW NOX (only 1 aerosol phase)
   !NOX = NLOWNOX ! store in spot 5
   Spc(I,J,L,id_IVSOAN) = MAX( AM0(5,JSV), 1e-32_fp )
   !----------------------------------------------------------

   !(mrq, 07/22)
   !--------------------------------------
   IF ( State_Diag%Archive_ProdSOAfromIVOCs ) THEN
         State_Diag%ProdSOAfromIVOCs(I,J,L) = sum(SPECSOAPROD(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   IF ( State_Diag%Archive_EvapSOAfromIVOCs ) THEN
         State_Diag%EvapSOAfromIVOCs(I,J,L) = sum(SPECSOAEVAP(I,J,L,:,JSV)) / DTCHEM
   ENDIF
   !--------------------------------------

   !=================================================================
   !(mrq, 01/22) change order from 4 to 5
   ! Semivolatile 5: POA/primary SVOCs  
   ...
   IF ( id_POA1 > 0 .and. id_POG1 > 0 .and. &
      ...
      Spc(I,J,L,id_POG3) = MAX( GM0(3,JSV), 1.e-32_fp ) !(mrq, 01/20)
      Spc(I,J,L,id_POG4) = MAX( GM0(4,JSV), 1.e-32_fp ) !(mrq, 01/20)
      ! aerosol phase
      Spc(I,J,L,id_POA1) = MAX( AM0(1,JSV), 1.e-32_fp )
      Spc(I,J,L,id_POA2) = MAX( AM0(2,JSV), 1.e-32_fp )
      Spc(I,J,L,id_POA3) = MAX( AM0(3,JSV), 1.e-32_fp ) !(mrq, 01/20)
      Spc(I,J,L,id_POA4) = MAX( AM0(4,JSV), 1.e-32_fp ) !(mrq, 01/20)
      Spc(I,J,L,id_POA5) = MAX( AM0(5,JSV), 1.e-32_fp ) !(mrq, 01/20)

      !(mrq, 07/22)
      !--------------------------------------
      IF ( State_Diag%Archive_ProdPOAfromPOGemit ) THEN
            State_Diag%ProdPOAfromPOGemit(I,J,L) = sum(SPECSOAPROD(I,J,L,:,JSV)) / DTCHEM
      ENDIF
      IF ( State_Diag%Archive_EvapPOAfromPOGemit ) THEN
            State_Diag%EvapPOAfromPOGemit(I,J,L) = sum(SPECSOAEVAP(I,J,L,:,JSV)) / DTCHEM
      ENDIF
      !--------------------------------------
   ...
   !=================================================================
   !(mrq, 01/22) change order from 5 to 6
   ! Semivolatile 6: OPOA/oxidized primary SVOCs 
   ...
   IF ( id_OPOA1 > 0 .and. id_OPOG1 > 0 .and. &
      ...
      Spc(I,J,L,id_OPOA3) = MAX( AM0(3,JSV), 1.e-32_fp ) !(mrq, 01/20)

      !(mrq, 07/22)
      !--------------------------------------
      IF ( State_Diag%Archive_ProdSOAfromSVOCs ) THEN
            State_Diag%ProdSOAfromSVOCs(I,J,L) = sum(SPECSOAPROD(I,J,L,:,JSV)) / DTCHEM
      ENDIF
      IF ( State_Diag%Archive_EvapSOAfromSVOCs ) THEN
            State_Diag%EvapSOAfromSVOCs(I,J,L) = sum(SPECSOAEVAP(I,J,L,:,JSV)) / DTCHEM
      ENDIF
      !--------------------------------------
...
!
   ...
   INTEGER                  :: HCOPOG3, HCOPOG4, HCOPOA5  !(mrq, 06/20)
   REAL(fp)                 :: EMIS, TMPFLX
   REAL(fp)                 :: F_OF_PBL
   LOGICAL                  :: FOUND
   ...
   IF ( Input_Opt%LSOA ) THEN
      ...
      !(mrq, 01/22) Get HEMCO ID of species POG3, POG4, and POA5
      !--------------------------------------------------------------
      HCOPOG3 = id_POG3
      IF ( HCOPOG3 > 0 ) THEN
         IF ( .NOT. ASSOCIATED(HcoState%Spc(HCOPOG3)%Emis%Val) ) THEN
            HCOPOG3 = -1
         ENDIF
      ENDIF
      HCOPOG4 = id_POG4
      IF ( HCOPOG4 > 0 ) THEN
         IF ( .NOT. ASSOCIATED(HcoState%Spc(HCOPOG4)%Emis%Val) ) THEN
            HCOPOG4 = -1
         ENDIF
      ENDIF
      HCOPOA5 = id_POA5
      IF ( HCOPOA5 > 0 ) THEN
         IF ( .NOT. ASSOCIATED(HcoState%Spc(HCOPOA5)%Emis%Val) ) THEN
            HCOPOA5 = -1
         ENDIF
      ENDIF
      !--------------------------------------------------------------
   ...
   DO I = 1, State_Grid%NX
      ...
      IF ( HCOPOG3 > 0 ) THEN
         ! Units from HEMCO are kgC/m2/s. Convert to kgC/box here.
         TMPFLX = SUM(HcoState%Spc(HCOPOG3)%Emis%Val(I,J,:)) &
                  * GET_TS_EMIS() * State_Grid%Area_M2(I,J)
         POAEMISS(I,J,L,3) = F_OF_PBL * TMPFLX
      ENDIF
      IF ( HCOPOG4 > 0 ) THEN
         ! Units from HEMCO are kgC/m2/s. Convert to kgC/box here.
         TMPFLX = SUM(HcoState%Spc(HCOPOG4)%Emis%Val(I,J,:)) &
                  * GET_TS_EMIS() * State_Grid%Area_M2(I,J)
         POAEMISS(I,J,L,4) = F_OF_PBL * TMPFLX
      ENDIF
      IF ( HCOPOA5 > 0 ) THEN
         ! Units from HEMCO are kgC/m2/s. Convert to kgC/box here.
         TMPFLX = SUM(HcoState%Spc(HCOPOA5)%Emis%Val(I,J,:)) &
                  * GET_TS_EMIS() * State_Grid%Area_M2(I,J)
         POAEMISS(I,J,L,5) = F_OF_PBL * TMPFLX
      ENDIF
      !----------------------------------------------------------------
...
!
   ...
   MOTEMP = Spc(I,J,L,id_ASOA1) + &   !(mrq, 01/22) Spc(I,J,L,id_ASOAN) + & 
            Spc(I,J,L,id_ASOA2) + &
            Spc(I,J,L,id_ASOA3) + &
            Spc(I,J,L,id_ASOA0) + &   !(mrq, 01/22)
            Spc(I,J,L,id_TSOA1) + &
            Spc(I,J,L,id_TSOA2) + &
            Spc(I,J,L,id_TSOA3) + &
            Spc(I,J,L,id_TSOA0) + &
            Spc(I,J,L,id_ISOA1) + &   !(mrq, 01/22)
            Spc(I,J,L,id_ISOA2) + &   !(mrq, 01/22)
            Spc(I,J,L,id_ISOA3) + &   !(mrq, 01/22)
            Spc(I,J,L,id_ISOA0) + &   !(mrq, 01/22)
            Spc(I,J,L,id_IVSOA1) + &   !(mrq, 01/22)
            Spc(I,J,L,id_IVSOA2) + &   !(mrq, 01/22)
            Spc(I,J,L,id_IVSOA3) + &   !(mrq, 01/22)
            Spc(I,J,L,id_IVSOA0) + &   !(mrq, 01/22)
            Spc(I,J,L,id_IVSOAN)       !(mrq, 01/22)
   ...
   IF ( id_POA1 > 0 ) THEN
      MOTEMP = MOTEMP              + &
               ...
               Spc(I,J,L,id_POA3 ) * OCFPOA(I,J)  + & !(mrq, 01/20)
               Spc(I,J,L,id_POA4 ) * OCFPOA(I,J)  + & !(mrq, 01/20)
               Spc(I,J,L,id_POA5 ) * OCFPOA(I,J)  + & !(mrq, 01/20)
               Spc(I,J,L,id_OPOA1) * OCFOPOA(I,J) + &
               Spc(I,J,L,id_OPOA2) * OCFOPOA(I,J) + &
               Spc(I,J,L,id_OPOA3) * OCFOPOA(I,J)    !(mrq, 01/20)
   ...
   IPR = 4
   ! Compute OA in kg
   OATEMP = Spc(I,J,L,id_ISOG0) * KOMIJL(IPR,JSV) * MOTEMP
   ! Compute difference in ug/m3
   EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_ISOA0) )* CONVFAC
   ! Assess error
   IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
      WRITE(*,*) 'EQLB Problem PR, JSV', IPR, JSV, ' in box ', I, J, L
   ENDIF
   ...
   !----------------------------------------------------
   !(mrq, 01/22) move IVOC to 4
   ! Semivolatile 3: lumped arom: total SOA+SOG in kg  
   !----------------------------------------------------
   JHC = PARENTBENZ
   JSV = IDSV(JHC)
   ...
   IPR = 4
   ! Compute OA in kg
   OATEMP = Spc(I,J,L,id_ASOG0) * KOMIJL(IPR,JSV) * MOTEMP
   ! Compute difference in ug/m3
   EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_ASOA0) )* CONVFAC
   ! Assess error
   IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
      WRITE(*,*) 'EQLB Problem PR, JSV', IPR, JSV, ' in box ', I, J, L
   ENDIF
   ...
   !-----------------------------------
   !----------------------------------------------------
   ! Semivolatile 4: lumped IVOC: total SOA+SOG in kg  
   !----------------------------------------------------
   JHC = PARENTNAP
   JSV = IDSV(JHC)
   ...
   IPR = 1
   ! Compute OA in kg
   OATEMP = Spc(I,J,L,id_IVSOG1) * KOMIJL(IPR,JSV) * MOTEMP
   ! Compute difference in ug/m3
   EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_IVSOA1) )* CONVFAC
   ! Assess error
   IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
      WRITE(*,*) 'EQLB Problem PR, JSV', IPR, JSV, ' in box ', I, J, L
   ENDIF
   ...
   IPR = 2
   ! Compute OA in kg
   OATEMP = Spc(I,J,L,id_IVSOG2) * KOMIJL(IPR,JSV) * MOTEMP
   ! Compute difference in ug/m3
   EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_IVSOA2) )* CONVFAC
   ! Assess error
   IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
      WRITE(*,*) 'EQLB Problem PR, JSV', IPR, JSV, ' in box ', I, J, L
   ENDIF
   ...
   IPR = 3
   ! Compute OA in kg
   OATEMP = Spc(I,J,L,id_IVSOG3) * KOMIJL(IPR,JSV) * MOTEMP
   ! Compute difference in ug/m3
   EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_IVSOA3) )* CONVFAC
   ! Assess error
   IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
      WRITE(*,*) 'EQLB Problem PR, JSV', IPR, JSV, ' in box ', I, J, L
   ENDIF
   ...
   IPR = 4
   ! Compute OA in kg
   OATEMP = Spc(I,J,L,id_IVSOG0) * KOMIJL(IPR,JSV) * MOTEMP
   ! Compute difference in ug/m3
   EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_IVSOA0) )* CONVFAC
   ! Assess error
   IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
      WRITE(*,*) 'EQLB Problem PR, JSV', IPR, JSV, ' in box ', I, J, L
   ENDIF
   ...
   IF ( id_POA1 > 0 ) THEN
      ...
      IPR = 3
      ! Compute OA in kg
      OATEMP = Spc(I,J,L,id_POG3) * KOMIJL(IPR,JSV) * MOTEMP
      ! Compute difference in ug/m3
      EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_POA3) )* CONVFAC
      ! Assess error
      IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
         WRITE(*,*) 'EQLB Problem NOX, PR, JSV', NOX, IPR, JSV, &
                    ' in box ', I, J, L
      ...
      IPR = 4
      ! Compute OA in kg
      OATEMP = Spc(I,J,L,id_POG4) * KOMIJL(IPR,JSV) * MOTEMP
      ! Compute difference in ug/m3
      EQLBDIFF = ABS( OATEMP - Spc(I,J,L,id_POA4) )* CONVFAC
      ! Assess error
      IF ( EQLBDIFF > ACCEPTERRORUG ) THEN
         WRITE(*,*) 'EQLB Problem NOX, PR, JSV', NOX, IPR, JSV, &
                    ' in box ', I, J, L
...
!
   ...
   DO I = 1, State_Grid%NX
      ...
      !(mrq, 01/22)
      OAGINITSAVE(I,J,L,4,JSV) = Spc(I,J,L,id_ISOA0) + Spc(I,J,L,id_ISOG0)
      !------------------------------------------------------------------------

      !----------------------------------------------------
      ! Semivolatile 3: lumped arom: total SOA+SOG in kg  !(mrq, 01/22) /IVOC
      !----------------------------------------------------
      JHC = PARENTBENZ
      JSV = IDSV(JHC)
      ! High NOx
      ...
      !(mrq, 01/22)
      OAGINITSAVE(I,J,L,4,JSV) = Spc(I,J,L,id_ASOA0) + Spc(I,J,L,id_ASOG0)
      ! Low NOx
      !(mrq, 01/22)
      ...
      JSV = IDSV(JHC)
      ! High NOx
      OAGINITSAVE(I,J,L,1,JSV) = Spc(I,J,L,id_IVSOA1) + Spc(I,J,L,id_IVSOG1)
      OAGINITSAVE(I,J,L,2,JSV) = Spc(I,J,L,id_IVSOA2) + Spc(I,J,L,id_IVSOG2)
      OAGINITSAVE(I,J,L,3,JSV) = Spc(I,J,L,id_IVSOA3) + Spc(I,J,L,id_IVSOG3)
      OAGINITSAVE(I,J,L,4,JSV) = Spc(I,J,L,id_IVSOA0) + Spc(I,J,L,id_IVSOG0)
      ! Low NOx
      OAGINITSAVE(I,J,L,5,JSV) = Spc(I,J,L,id_IVSOAN)
      !----------------------------------------------------
      ...
      IF ( id_POA1 > 0 ) THEN
         ...
         !(mrq, 06/20)
         !------------------------------------------
         OAGINITSAVE(I,J,L,3,JSV) = Spc(I,J,L,id_POA3) + Spc(I,J,L,id_POG3)
         OAGINITSAVE(I,J,L,4,JSV) = Spc(I,J,L,id_POA4) + Spc(I,J,L,id_POG4)
         OAGINITSAVE(I,J,L,5,JSV) = Spc(I,J,L,id_POA5)
         !------------------------------------------
      ENDIF
      ...
      IF ( id_OPOA1 > 0 ) THEN
         ...
         OAGINITSAVE(I,J,L,3,JSV) = Spc(I,J,L,id_OPOA3)    !(mrq, 06/20)
      ENDIF
...
!       ENDIF
      ...
      !(mrq, 01/22) Product 4, C* = 0.1 ug/m3
      IPR      = 4
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + SUM(TEMPDELTA(:,IPR))
      MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_ISOA0) + Spc(I,J,L,id_ISOG0) ))
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!          print*,'DELSOGSAVE NOx=2',DELTASOGSAVE(I,J,L,2,5)
!          print*,'DELSOGSAVE NOx=3',DELTASOGSAVE(I,J,L,3,5)
!          print*,'Spc',Spc(I,J,L,id_ISOA0),Spc(I,J,L,id_ISOG0)
!          print*,'NNOX',NNOX(JSV)
!          print*,'strat?',State_Met%InStratosphere(I,J,L)
!       ENDIF
      !------------------------------------------------------------------------
...
!       ENDIF
      ...
      IPR      = 4
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
      MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_ASOA0) + Spc(I,J,L,id_ASOG0) ))
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!       ENDIF
      ...
      DO NOX = 1, NNOX(JSV)
         TEMPDELTA(NOX,IPR) = &
           DELTASOGSAVE(I,J,L,NOX,PARENTNAP )*ALPHA(NOX,IPR,PARENTNAP )   &
         + DELTASOGSAVE(I,J,L,NOX,PARENTIVOC5)*ALPHA(NOX,IPR,PARENTIVOC5) & 
         + DELTASOGSAVE(I,J,L,NOX,PARENTIVOC4)*ALPHA(NOX,IPR,PARENTIVOC4)
      ENDDO
      ENDDO
      ...
      IPR      = 5
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
      MBDIFF   = ABS( TEMPSOAG - Spc(I,J,L,id_IVSOAN) )
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!       ENDIF
      ...
      IPR      = 1
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
      MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_IVSOA1) + Spc(I,J,L,id_IVSOG1) ))
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!       ENDIF
      ...
      IPR      = 2
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
      MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_IVSOA2) + Spc(I,J,L,id_IVSOG2) ))
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!       ENDIF
      ...
      IPR      = 3
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
      MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_IVSOA3) + Spc(I,J,L,id_IVSOG3) ))
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!       ENDIF
      ...
      IPR      = 4
      TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
      MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_IVSOA0) + Spc(I,J,L,id_IVSOG0) ))
      MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!       IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!          WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, JSV, &
!                     'in box ', I, J, L
...
!          ENDIF
         ...
         IPR      = 3
         TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
         MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_POA3) + Spc(I,J,L,id_POG3) ))
         MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!          IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!             WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, &
!                        JSV, 'in box ', I, J, L
...
!          ENDIF
         ...
         IPR      = 4
         TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
         MBDIFF   = ABS( TEMPSOAG - ( Spc(I,J,L,id_POA4) + Spc(I,J,L,id_POG4) ))
         MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!          IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!             WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, &
!                        JSV, 'in box ', I, J, L
...
!          ENDIF
         ...
         IPR      = 5
         TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
         MBDIFF   = ABS( TEMPSOAG - Spc(I,J,L,id_POA5))
         MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!          IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!             WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, &
!                        JSV, 'in box ', I, J, L
...
!          ENDIF
         ...
         IPR      = 3
         TEMPSOAG = OAGINITSAVE(I,J,L,IPR,JSV) + TEMPDELTA(NOX,IPR)
         MBDIFF   = ABS( TEMPSOAG - Spc(I,J,L,id_OPOA3))
         MBDIFF   = MBDIFF/TEMPSOAG ! convert to fractional error
!          IF ( prtDebug .and. MBDIFF > ACCEPTERROR ) THEN
!             WRITE(*,*) 'MB Problem with NOX, IPR, JSV:', NOX, IPR, &
!                        JSV, 'in box ', I, J, L
...
!       print*,'ISOP NO3      Rxn : ', DELTAHCSAVE(2,JHC)
      ...
      JHC = 12   !(mrq, 05/20)
      print*,'IVOC5  High NOx Rxn : ', DELTAHCSAVE(1,JHC)
      print*,'IVOC5  Low  NOx Rxn : ', DELTAHCSAVE(2,JHC)
      JHC = 13   !(mrq, 05/20)
      print*,'IVCO4  High NOx Rxn : ', DELTAHCSAVE(1,JHC)
      print*,'IVOC4  Low  NOx Rxn : ', DELTAHCSAVE(2,JHC)
!       JHC = 10
!       print*,'POG1 OH       Rxn : ', DELTAHCSAVE(1,JHC)
!       print*,'POG2 OH       Rxn : ', DELTAHCSAVE(2,JHC)
!       print*,'POG3 OH       Rxn : ', DELTAHCSAVE(3,JHC) !(mrq, 01/20)
!       print*,'POG4 OH       Rxn : ', DELTAHCSAVE(4,JHC) !(mrq, 01/20)
...
!       !------------------------------------------------------------------------
      ...
      print*,'ASOA3 prod and evap: ', &
         SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      IPR = 4
      !print*,'ASOAN prod and evap: ', &   !(mrq, 01/22)
      print*,'ASOA0 prod and evap: ', &
         SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      ...
      JSV = 4
      IPR = 1
      print*,'IVSOA1 prod and evap: ', &
         SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      IPR = 2
      print*,'IVSOA2 prod and evap: ', &
        SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      IPR = 3
      print*,'IVSOA3 prod and evap: ', &
         SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      IPR = 4
      print*,'IVSOA0 prod and evap: ', &
         SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      IPR = 5
      print*,'IVSOAN prod and evap: ', &
         SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
      !-----------------------------------

!       JSV = 5 !4 !(mrq, 01/22) from 4 to 5
!       IPR = 1
...
!       !-------------------------------------
!       IPR = 3
!       print*,'POA3  prod and evap: ', &
!          SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
!       IPR = 4
!       print*,'POA4  prod and evap: ', &
!          SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
!       IPR = 5
!       print*,'POA5  prod and evap: ', &
!          SUM(SPECSOAPROD(:,:,:,IPR,JSV)),SUM(SPECSOAEVAP(:,:,:,IPR,JSV))
!       !-------------------------------------

!       JSV = 6 !5 !(mrq, 01/22) from 5 to 6
!       IPR = 1
...
!
   ...
   id_ASOG2    = IND_('ASOG2'   )
   id_ASOG3    = IND_('ASOG3'   )
   id_ASOG0    = IND_('ASOG0'   )   !(mrq, 02/20)
   id_ASOA1    = IND_('ASOA1'   )
   id_ASOA2    = IND_('ASOA2'   )
   id_ASOA3    = IND_('ASOA3'   )
   !id_ASOAN    = IND_('ASOAN'   )  !(mrq, 01/22) remove ASOAN   
   id_ASOA0    = IND_('ASOA0'   )   !(mrq, 02/20)
   !(mrq, 01/22)
   !-----------------------------
   id_IVSOG1   = IND_('IVSOG1'  )
   id_IVSOG2   = IND_('IVSOG2'  )
   id_IVSOG3   = IND_('IVSOG3'  )
   id_IVSOG0   = IND_('IVSOG0'  ) 
   id_IVSOA1   = IND_('IVSOA1'  )
   id_IVSOA2   = IND_('IVSOA2'  )
   id_IVSOA3   = IND_('IVSOA3'  )
   id_IVSOA0   = IND_('IVSOA0'  )
   id_IVSOAN   = IND_('IVSOAN'  )
   id_ISOG1    = IND_('ISOG1'   )
   id_ISOG2    = IND_('ISOG2'   )
   id_ISOG3    = IND_('ISOG3'   )
   id_ISOG0    = IND_('ISOG0'   ) 
   ...
   id_IVOC5     = IND_('IVOC5'    )
   id_IVOC4     = IND_('IVOC4'    )
   id_LNRO2N5   = IND_('LNRO2N5'  )
   id_LNRO2H5   = IND_('LNRO2H5'  )
   id_LNRO2N4   = IND_('LNRO2N4'  )
   id_LNRO2H4   = IND_('LNRO2H4'  )
   !------------------------------
   ...
   ! isoprene
   IDSV(PARENTISOP) = 2
   ! Lumped arom !/IVOC (mrq, 01/22) move IVOC to 4
   IDSV(PARENTBENZ) = 3
   IDSV(PARENTTOLU) = 3
   IDSV(PARENTXYLE) = 3
   ! Lumped IVOC (mrq, 01/22)
   IDSV(PARENTNAP ) = 4
   IDSV(PARENTIVOC5 ) = 4  !(mrq, 01/22)
   IDSV(PARENTIVOC4 ) = 4  !(mrq, 01/22)
   ! More individuals
   IDSV(PARENTPOA ) = 5 !4 !(mrq, 01/22) from 4 to 5
   IDSV(PARENTOPOA) = 6 !5 !(mrq, 01/22) from 5 to 6

   ! Define number of products per semivolatile (hotp 5/14/10)
   NPROD(IDSV(PARENTMTPA)) = 4 ! 3 add C*=0.1 product (hotp 6/12/10)
   NPROD(IDSV(PARENTISOP)) = 4 ! 3 (mrq, 01/22) from 3 to 4, add C*=0.1 product
   NPROD(IDSV(PARENTBENZ)) = 4
   NPROD(IDSV(PARENTNAP))  = 5  !(mrq, 01/22)
   NPROD(IDSV(PARENTPOA )) = 5  !2  !(mrq, 02/20) C*=0.01-100
   NPROD(IDSV(PARENTOPOA)) = 3  !2  !(mrq, 02/20) C*=0.01-1
   ...
   NNOX(IDSV(PARENTISOP)) = 3 !2 ! low  NOx, NO3  !(mrq, 01/22) add high NOx
   NNOX(IDSV(PARENTBENZ)) = 2 ! high NOx, low NOx
   NNOX(IDSV(PARENTNAP))  = 2 ! high NOx, low NOx !(mrq, 01/22) 
   ...
   !(mrq, 01/22)  from 2 to 5
   !ALLOCATE( POAEMISS(State_Grid%NX,State_Grid%NY,State_Grid%NZ,2),STAT=AS )
   ALLOCATE( POAEMISS(State_Grid%NX,State_Grid%NY,State_Grid%NZ,5),STAT=AS )
   ...
   IF ( Input_Opt%LSOA ) THEN
      ...
      !(mrq, 01/22) increase 2 dimension by adding IVOC4 and IVOC5 (from 4 to 6)
      ALLOCATE( GLOB_DARO2(State_Grid%NX,State_Grid%NY,State_Grid%NZ,2,6), &
                STAT=AS )
      IF ( AS /= 0 ) CALL ALLOC_ERR( 'GLOB_DARO2' )
      GLOB_DARO2 = 0e+0_fp

      ! semivolpoa4: diagnostic (hotp 3/27/09)
      !(mrq, 01/22) from 2 to 4
      ALLOCATE( GLOB_POGRXN(State_Grid%NX,State_Grid%NY,State_Grid%NZ, 4), &
                STAT=AS )
