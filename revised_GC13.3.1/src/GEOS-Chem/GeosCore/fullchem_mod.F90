    ...
    DO N = 1, State_Chm%nSpecies
       ...
       IF ( Input_Opt%LSOA .or. Input_Opt%LSVPOA ) THEN
          SELECT CASE ( TRIM( SpcInfo%Name ) )
             !(mrq, 05/20) LNRO2H and LNRO2N for IVOC5 and IVOC4 
             CASE ( 'LBRO2H', 'LBRO2N', 'LTRO2H', 'LTRO2N', &
                    'LXRO2H', 'LXRO2N', 'LNRO2H', 'LNRO2N', & 
                    'LNRO2H5','LNRO2N5','LNRO2H4','LNRO2N4')    !(mrq, 05/20)
                State_Chm%Species(:,:,:,N) = 0e+0_fp
          END SELECT
       ENDIF
...
!
    ...
    REAL(f8)           :: SgVtemp   !(mrq, 12/21)

    ! Pointers (mrq, 10/20)
    REAL(fp), POINTER :: URFLAC(:,:)
    REAL(fp), POINTER :: MLAI(:,:)
    ...
    !--------------------------------
    ! J-values of NO2 [s-1]
    RADIAT            = State_Met%SWGDN(I,J)               
    ! calculate the ground surface to volume ratio (only in the first layer)
    IF ( L .eq. 1 ) THEN
        URFLAC          => State_Met%UrbanFrac
        MLAI            => State_Met%MODISLAI
        SgVtemp = ( 1 - URFLAC(I,J) ) * ( (2.0_fp * MLAI(I,J)) / State_Met%BXHEIGHT(I,J,1) ) &
                    + URFLAC(I,J) * 0.1_fp
        SgV = max( SgVtemp, 1.0_fp/ State_Met%BXHEIGHT(I,J,1) )
    ELSE 
        SgV = 0
    ENDIF            
    !--------------------------------
