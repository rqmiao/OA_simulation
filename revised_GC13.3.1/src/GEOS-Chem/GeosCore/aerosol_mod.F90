...
!
  ...
  REAL(fp), ALLOCATABLE, PUBLIC :: ISOA(:,:,:)   !(mrq, 01/22)
  REAL(fp), ALLOCATABLE, PUBLIC :: IVSOA(:,:,:)  !(mrq, 01/22)
  REAL(fp), ALLOCATABLE, PUBLIC :: MOPI(:,:,:)   !(mrq, 01/22)
  REAL(fp), ALLOCATABLE, PUBLIC :: MOPO(:,:,:)   !(mrq, 01/22)
  ...
  REAL(fp), ALLOCATABLE, PUBLIC :: OCFNPOA(:,:)   !(mrq, 01/22)
...
!
  ...
  INTEGER :: id_SO4,   id_SO4s,  id_NITs             !(mrq, 01/22)  id_NH4s
  INTEGER :: id_POA1,  id_POA2,  id_OPOA1, id_OPOA2
  INTEGER :: id_TSOA1, id_TSOA2, id_TSOA3, id_TSOA0
  INTEGER :: id_ASOA1, id_ASOA2, id_ASOA3            ! remove id_ASOAN (mrq, 01/22)
  INTEGER :: id_DUST1, id_SOAS,  id_SALACL, id_HMS   ! (jmm, 06/29/18)
  INTEGER :: id_SOAGX, id_SOAIE
  INTEGER :: id_INDIOL,id_LVOCOA
  INTEGER :: id_POA3,  id_POA4,  id_POA5             !(mrq, 09/20)
  INTEGER :: id_OPOA3                                !(mrq, 09/20)
  INTEGER :: id_ASOA0                                !(mrq, 01/22)
  INTEGER :: id_ISOA1, id_ISOA2, id_ISOA3, id_ISOA0  !(mrq, 01/22)
  INTEGER :: id_IVSOA1, id_IVSOA2, id_IVSOA3, id_IVSOA0  !(mrq, 01/22)
  INTEGER :: id_IVSOAN                                   !(mrq, 01/22)
  INTEGER :: id_MOPO, id_MOPI                            !(mrq, 01/22)
...
!
    ...
    LOGICAL             :: IS_ISOA,  IS_IVSOA   !(mrq, 01/22)
    LOGICAL             :: IS_MOPO,  IS_MOPI    !(mrq, 01/22)
    ...
    !(mrq, 01/22)
    !-------------------------------------
    IS_ISOA    = ( id_ISOA1 > 0 .AND. id_ISOA2 > 0 .AND. &
                   id_ISOA3 > 0 .AND. id_ISOA0 > 0 )
    IS_IVSOA   = ( id_IVSOA1 > 0 .AND. id_IVSOA2 > 0 .AND. &
                   id_IVSOA3 > 0 .AND. id_IVSOA0 > 0 .AND. &
                   id_IVSOAN > 0)
    !-------------------------------------
    IS_ASOA    = ( id_ASOA1 > 0 .AND. &                      ! remove id_ASOAN (mrq, 01/22)
                   id_ASOA2 > 0 .AND. id_ASOA3 > 0 .AND. &
                   id_ASOA0 > 0 )                            !(mrq, 01/22)
    IS_POA     = ( id_POA1  > 0 .AND. id_POA2  > 0 .AND. &
                   id_POA3  > 0 .AND. id_POA4  > 0 .AND. &   !(mrq, 01/22)
                   id_POA5  > 0)                             !(mrq, 01/22)
    IS_OPOA    = ( id_OPOA1 > 0 .AND. id_OPOA2 > 0 .AND. &
                   id_OPOA3 > 0 )                            !(mrq, 01/22)
    IS_SOAGX   = ( id_SOAGX > 0 )
    IS_MOPI    = ( id_MOPI  > 0 )                            !(mrq, 01/22)
    IS_MOPO    = ( id_MOPO  > 0 )                            !(mrq, 01/22)
    ...
    IF ( RC == GC_SUCCESS .AND. FND ) THEN
       ...
       OCFNPOA(:,:) = OCFPOA(:,:)   !(mrq, 01/22)

    ELSE

       ! Use default global mean OM/OC recommended by the Aerosols WG
       OCFPOA(:,:)  = 1.4e+0_fp ! OM/OC for semivolatile POA (mrq, 01/22)
       OCFOPOA(:,:) = 2.1e+0_fp ! OM/OC for OPOA, OCPI, and OCPO
       OCFNPOA(:,:) = 1.6e+0_fp ! OM/OC for nonvolatile POA (mrq, 01/22)
    ...
    DO I = 1, State_Grid%NX
       ...
       IF ( LCARB ) THEN
          ...
          IF ( IS_POA ) THEN
             ...
             !(mrq, 12/21)
             OCPO(I,J,L) = ( Spc(I,J,L,id_POA1) + Spc(I,J,L,id_POA2) + &
                             Spc(I,J,L,id_POA3) + Spc(I,J,L,id_POA4) + &
                             Spc(I,J,L,id_POA5) ) &
                           * OCFPOA(I,J) / AIRVOL(I,J,L)
          ...
          !(mrq, 01/22) Hydrophobic marine OC [kg/m3]
          IF ( IS_MOPO ) THEN
             MOPO(I,J,L) = Spc(I,J,L,id_MOPO) * OCFNPOA(I,J) / AIRVOL(I,J,L)
          ENDIF

          !(mrq, 01/22) Hydrophilic marine OC [kg/m3] OCFNPOA(I,J)
          IF ( IS_MOPI ) THEN
             MOPI(I,J,L) = Spc(I,J,L,id_MOPI) * OCFNPOA(I,J) / AIRVOL(I,J,L)
          ENDIF
          ...
          MOPI(I,J,L)    = MAX( MOPI(I,J,L), 1e-35_fp )   !(mrq, 01/22)
          MOPO(I,J,L)    = MAX( MOPO(I,J,L), 1e-35_fp )   !(mrq, 01/22)
       ...
       IF ( Is_ComplexSOA ) THEN
          ...
          IF ( IS_ASOA ) THEN
             ASOA(I,J,L) = ( Spc(I,J,L,id_ASOA1) + &            ! remove Spc(I,J,L,id_ASOAN) (mrq, 01/22)
          ...
          IF ( IS_ISOA ) THEN
             ISOA(I,J,L) = ( Spc(I,J,L,id_ISOA1) + Spc(I,J,L,id_ISOA2) + &
                             Spc(I,J,L,id_ISOA3) + Spc(I,J,L,id_ISOA0) ) &
                             / AIRVOL(I,J,L)
          ENDIF

          ! IVSOA (IVOC SOA) [kg/m3]
          IF ( IS_IVSOA ) THEN
             IVSOA(I,J,L) = ( Spc(I,J,L,id_IVSOA1) + Spc(I,J,L,id_IVSOA2) + &
                              Spc(I,J,L,id_IVSOA3) + Spc(I,J,L,id_IVSOA0) + &
                              Spc(I,J,L,id_IVSOAN) ) &
                             / AIRVOL(I,J,L)
          ENDIF
          !------------------------------------
       ...
       ELSEIF ( Is_ComplexSOA ) THEN
          ...
          OCPISOA(I,J,L) = ( Spc(I,J,L,id_TSOA1) + Spc(I,J,L,id_TSOA2) + &
                             Spc(I,J,L,id_TSOA3) + Spc(I,J,L,id_TSOA0) + &
                             Spc(I,J,L,id_ASOA1) + Spc(I,J,L,id_ASOA2) + &     !(mrq, 01/22) remove Spc(I,J,L,id_ASOAN) 
                             Spc(I,J,L,id_ASOA3) + Spc(I,J,L,id_ASOA0) + &
                             Spc(I,J,L,id_ISOA1) + Spc(I,J,L,id_ISOA2) + &     !(mrq, 01/22)
                             Spc(I,J,L,id_ISOA3) + Spc(I,J,L,id_ISOA0) + &     !(mrq, 01/22)
                             Spc(I,J,L,id_IVSOA1) + Spc(I,J,L,id_IVSOA2) + &     !(mrq, 01/22)
                             Spc(I,J,L,id_IVSOA3) + Spc(I,J,L,id_IVSOA0) + &     !(mrq, 01/22)
       ...
       ELSEIF ( Is_ComplexSOA ) THEN
          PM25(I,J,L) = PM25(I,J,L)                + &
                        ...
                        ISOA(I,J,L)   * ORG_GROWTH + &   !(mrq, 01/22)
                        IVSOA(I,J,L)  * ORG_GROWTH + &   !(mrq, 01/22)
       ...
       !-----------------------------------------------
       IF ( IS_MOPO ) THEN
          PM25(I,J,L) = PM25(I,J,L) + MOPO(I,J,L)
       ENDIF
       IF ( IS_MOPI ) THEN
          PM25(I,J,L) = PM25(I,J,L) + MOPI(I,J,L) * ORG_GROWTH
       ENDIF
       !-----------------------------------------------
...
110       FORMAT( '     - RDAER: Using online BCPI OCPI BCPO OCPO!' )
       ...
       DO I = 1, State_Grid%NX
          ...
          !(mrq, 09/20) 50% POA is hydrophilic
          WAERSL(I,J,L,3) = OCPISOA(I,J,L) + OCPO(I,J,L)*0.5e+0_fp + &
                            MOPI(I,J,L)
          ...
          !(mrq, 09/20) 50% POA is hydrophilic
          DAERSL(I,J,L,2) = OCPO(I,J,L)*0.5e+0_fp + MOPO(I,J,L)
...
!
    ...
    id_OCPO   = Ind_( 'OCPO'   )
    id_OCPI   = Ind_( 'OCPI'   )
    id_MOPO   = Ind_( 'MOPO'   )  !(mrq, 01/22)
    id_MOPI   = Ind_( 'MOPI'   )  !(mrq, 01/22)
    id_SOAS   = Ind_( 'SOAS'   )
    id_SALA   = Ind_( 'SALA'   )
    id_SALC   = Ind_( 'SALC'   )
    ...
    id_POA1   = Ind_( 'POA1'   )
    id_POA2   = Ind_( 'POA2'   )
    id_POA3   = Ind_( 'POA3'   )  !(mrq, 09/20)
    id_POA4   = Ind_( 'POA4'   )  !(mrq, 09/20)
    id_POA5   = Ind_( 'POA5'   )  !(mrq, 09/20)
    id_OPOA1  = Ind_( 'OPOA1'  )
    id_OPOA2  = Ind_( 'OPOA2'  )
    id_OPOA3  = Ind_( 'OPOA3'  )  !(mrq, 09/20)
    id_TSOA1  = Ind_( 'TSOA1'  )
    id_TSOA2  = Ind_( 'TSOA2'  )
    id_TSOA3  = Ind_( 'TSOA3'  )
    id_TSOA0  = Ind_( 'TSOA0'  )
    ...
    id_ASOA2  = Ind_( 'ASOA2'  )
    id_ASOA3  = Ind_( 'ASOA3'  )
    id_ASOA0  = Ind_( 'ASOA0'  )  !(mrq, 01/22)
    id_ISOA1  = Ind_( 'ISOA1'  )  !(mrq, 01/22)
    id_ISOA2  = Ind_( 'ISOA2'  )  !(mrq, 01/22)
    id_ISOA3  = Ind_( 'ISOA3'  )  !(mrq, 01/22)
    id_ISOA0  = Ind_( 'ISOA0'  )  !(mrq, 01/22)
    id_IVSOA1 = Ind_( 'IVSOA1' )  !(mrq, 01/22)
    id_IVSOA2 = Ind_( 'IVSOA2' )  !(mrq, 01/22)
    id_IVSOA3 = Ind_( 'IVSOA3' )  !(mrq, 01/22)
    id_IVSOA0 = Ind_( 'IVSOA0' )  !(mrq, 01/22)
    id_IVSOAN = Ind_( 'IVSOAN' )  !(mrq, 01/22)
    id_SOAGX  = Ind_( 'SOAGX'  )
    id_SOAIE  = Ind_( 'SOAIE'  )
    ...
    !(mrq, 01/22)
    !-----------------------------------------
    ALLOCATE( MOPO( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:MOPO', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    MOPO = 0.0_fp

    ALLOCATE( MOPI( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:MOPI', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    MOPI = 0.0_fp
    !-----------------------------------------
    ...
    !(mrq, 01/22)
    !-----------------------------------
    ALLOCATE( ISOA( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:ISOA', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    ISOA = 0.0_fp

    ALLOCATE( IVSOA( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:IVSOA', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    IVSOA = 0.0_fp
    !-----------------------------------
    ...
    !(mrq, 01/22) OM/OC for nonvolatile POA
    ALLOCATE( OCFNPOA(State_Grid%NX,State_Grid%NY), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:OCFNPOA', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    OCFNPOA = 0.0_fp
...
!
    ...
    IF ( ALLOCATED( MOPI        ) ) DEALLOCATE( MOPI        )  !(mrq, 01/22)
    IF ( ALLOCATED( MOPO        ) ) DEALLOCATE( MOPO        )  !(mrq, 01/22)
    ...
    IF ( ALLOCATED( ISOA        ) ) DEALLOCATE( ISOA        )  !(mrq, 01/22)
    IF ( ALLOCATED( IVSOA       ) ) DEALLOCATE( IVSOA       )  !(mrq, 01/22)
    ...
    IF ( ALLOCATED( OCFNPOA     ) ) DEALLOCATE( OCFNPOA     )  !(mrq, 01/22)
