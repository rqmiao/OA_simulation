...
!
  ...
  PUBLIC  :: Init_UrbanFrac         !(mrq, 12/21)
...
!BOP
!
! !IROUTINE: init_urbanfrac  (mrq, 12/21)
!
! !DESCRIPTION: Attaches pointers from the MODIS XLAI data read in by
!  HEMCO to the UrbanFrac field of State\_Met.
!\\
!\\
! !INTERFACE:
!
  SUBROUTINE Init_UrbanFrac( Input_Opt, State_Grid, State_Met, RC )
!
! !USES:
!
    USE ErrCode_Mod
...
!
    ...
    ErrMsg  = ''
    ThisLoc = &
      ' -> at Init_UrbanFrac (in module "GeosCore/olson_landmap_mod.F90'

    ! Get the HEMCO pointer to MODIS landtype mask
    ! (variable names are LANDTYPE00, LANDTYPE01 .. LANDTYPE72)
    Name = 'URBANLAND'
    CALL HCO_GC_GetPtr( Input_Opt, State_Grid, Name, Ptr2D, RC, FOUND=FND )
    ...
    ! Copy into State_Met%LandTypeFrac
    State_Met%UrbanFrac(:,:) = Ptr2D

    ! Free pointer
    Ptr2D => NULL()

  END SUBROUTINE Init_UrbanFrac
!EOC
