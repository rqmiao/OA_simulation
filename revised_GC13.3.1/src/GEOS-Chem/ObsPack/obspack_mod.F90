...
!
    ...
    !(mrq, 12/21)
    !---------------------------------------------
    ALLOCATE( State_Diag%ObsPack_AIRDEN( nObs ), STAT=RC )
    CALL GC_CheckVar( 'State_Diag%ObsPack_AIRDEN', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    State_Diag%ObsPack_AIRDEN = 0.0_f4
    !---------------------------------------------
 ...
 50 FORMAT( '     - OBSPACK: "time" indicates ', a, ' of sample window' )
    ...
    DO N = 1, nObs
       ...
       SELECT CASE ( State_Diag%ObsPack_Strategy(N) )
          ...
          !(mrq, 12/21) for calculating 20 min avg 
          !-----------------------------------------------
          !------------------
          ! 20-minute window
          !------------------
          CASE( 5 )
             ...
             IF ( Window_Start_Time ) THEN
                State_Diag%ObsPack_Ival_Center(N) =                           &
                     State_Diag%ObsPack_Ival_Start(N)  + 600.0_f8
             ELSE

                State_Diag%ObsPack_Ival_Start(N) =                           &
                     State_Diag%ObsPack_Ival_Center(N) - 600.0_f8
             ENDIF
...
!BOC
    ...
    !(mrq, 12/21)
    !----------------------------------------------
    IF ( ASSOCIATED( State_Diag%ObsPack_AIRDEN ) ) THEN
       DEALLOCATE( State_Diag%ObsPack_AIRDEN, STAT=RC )
       CALL GC_CheckVar( 'State_Diag%ObsPack_AIRDEN', 2, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Diag%ObsPack_AIRDEN => NULL()
    ENDIF
    !----------------------------------------------
...
!
    ...
    DO N = 1, nObs
       ...
       IF ( nSamples > 0 ) THEN
          ...
          !(mrq, 12/21)
          !-----------------------------------
          State_Diag%ObsPack_AIRDEN(N) = State_Diag%ObsPack_AIRDEN(N) / nSamples
          !-----------------------------------
...
!    CALL NcDef_Var_Attributes( fId, vId, 'units',     'degrees_east'        )
    ...
    CALL NcDef_Variable( fId, 'air_density', NF_FLOAT, 1, dims_1d, vId      )
    CALL NcDef_Var_Attributes( fId, vId, 'long_name', 'Dry air density'     )
    CALL NcDef_Var_Attributes( fId, vId, 'units',     'kg m-3'              )
    !--------------------------------------
    ...
    !-----------------------------------------------
    varName = 'air_density'
    CALL NcWr( State_Diag%ObsPack_AIRDEN,   fId, TRIM(varName), st1d, ct1d  )
    !-----------------------------------------------
 ...
 100   FORMAT( 'OBSPACK SAMPLING at ', i4,   '/', i2.2, '/', i2.2,           &
    ...
    DO N = 1, State_Diag%ObsPack_nObs
       ...
       SELECT CASE ( State_Diag%ObsPack_Strategy(N) )
          ...
          !(mrq, 12/21) add case 5 for 20-min avg
          !----------------------------------
          CASE ( 5 )
             ! If the sample covers the entire dynamic timestep, then...
             IF ( State_Diag%ObsPack_Ival_Start(N) <= TsStart .and.                &
       ...
       IF ( doSample ) THEN
          ...
          !(mrq, 12/21)
          !-------------------------------------------------------
          State_Diag%ObsPack_AIRDEN(N) = State_Diag%ObsPack_AIRDEN(N)            &
