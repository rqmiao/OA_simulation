...
CONTAINS
  ...
  !----------------------------------------------------------------------
  !=========================================================================
  ! Hetchem rate-law functions for SO2
  !=========================================================================

  FUNCTION SO2uptk1stOrd( H ) RESULT( k )
    !
    ! Computes the reaction rate [1/s] for 1st order uptake of SO2.
    !
    TYPE(HetState), INTENT(IN) :: H              ! HetChem State
    REAL(dp)                   :: srMw, k        ! sqrt(mol wt), rxn rate [1/s]
...
!------------------------------------------------------------------------------
  ...
  FUNCTION NO2uptk1stOrdAndCloud( H ) RESULT( k )
    ...
    ! (mrq, 12/21) modify from 5e-6_dp to 1e-8_dp
    gamma = 5e-6_dp
    k = k + Ars_L1k( H%xArea(SUL), H%xRadi(SUL), gamma, srMw )
    !
    ! Uptake by black carbon (aerosol type 9)
    ! (mrq, 12/21) modify from 1e-4_dp to 1e-5_dp
    gamma = 1e-5_dp
    k = k + Ars_L1k( H%xArea(BKC), H%xRadi(BKC), gamma, srMw )
    !
  ...
  !(mrq, 12/21) heterogeneous reaction of NO2 on the ground 
  !---------------------------------------------------
  FUNCTION NO2uptk1stOrdOnGround( H ) RESULT( k )
    !
    ! Computes the reaction rate [1/s] for 1st-order uptake of NO2.
    !
    TYPE(HetState), INTENT(IN) :: H              ! HetChem State
    ...
    IF ( SUNCOS > 0.0_dp ) THEN
         gamma = 1.0e-5_dp * ( RADIAT/400.0_dp ) * (relhum/60.0_dp)
         !gamma = max(gamma, 1.0e-7_dp)
    ELSE
         gamma = 3.1e-7_dp * (relhum/70.0_dp)    
    ENDIF

    k = (gamma * SR_TEMP * SgV) / (2.749064e-4_dp*(srMw**0.5_dp)*2*100)

  END FUNCTION NO2uptk1stOrdOnGround
  !---------------------------------------------------
