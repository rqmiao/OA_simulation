...
!
  ...
  INTEGER,           PARAMETER :: N_SPEC  = 37 ! increase from 34 (v12.5.0 default)
                                               ! to 35 for MOH
                                               ! to 36 from HONO (mrq, 12/21)
                                               ! to 37 from NAP  (mrq, 12/21)
!
! !PRIVATE TYPES:
!
  TYPE :: MyInst
   ...
   REAL(sp)                       :: BCPIfrac
   REAL(sp)                       :: POG1frac
   REAL(sp)                       :: POG2frac    !(mrq, 01/20)
   REAL(sp)                       :: POG3frac    !(mrq, 01/20)
   REAL(sp)                       :: POG4frac    !(mrq, 01/20)
   REAL(sp)                       :: POA5frac    !(mrq, 06/20)
   REAL(sp)                       :: SOAPfrac
   REAL(sp)                       :: NAPfrac     !(mrq, 05/20)
   REAL(sp)                       :: IVOC5frac   !(mrq, 05/20)
   REAL(sp)                       :: IVOC4frac   !(mrq, 05/20)
...
!==============================================================================
       ...
       SELECT CASE ( Inst%SpcNames(N) )
          ...
          CASE ( 'POG2' )
             ...
             SpcArr = SpcArr * Inst%POG2frac !(mrq, 01/20)
          !(mrq, 01/20)
          !--------------------------------------
          CASE ( 'POG3' )
             SpcArr = SpcArr * Inst%POG3frac
          CASE ( 'POG4' )
             SpcArr = SpcArr * Inst%POG4frac
          CASE ( 'POA5' )
             SpcArr = SpcArr * Inst%POA5frac                                    
          !--------------------------------------
          CASE ( 'SOAP' )
          ...
          !--------------------------------------
          CASE ( 'NAP' )
             SpcArr = SpcArr * Inst%NAPfrac
          CASE ( 'IVOC5' )
             SpcArr = SpcArr * Inst%IVOC5frac
          CASE ( 'IVOC4' )
             SpcArr = SpcArr * Inst%IVOC4frac                                    
          !--------------------------------------
!==============================================================================
...
!
   ...
   !=================================================================
    ...
    !(mrq, 06/20)
    Inst%NAPfrac = 0.538
    Inst%IVOC5frac = 0.420
    Inst%IVOC4frac = 0.042
    !------------------------------------------ 
...
#include "hcox_gfed_include_gfed4.H"
    ...
    IF ( HcoState%amIRoot ) THEN
       ...
       !(mrq, 01/20)
       !-------------------------------------------------------
       WRITE(MSG,*) '   - POG2 fraction           : ', Inst%POG2frac
       CALL HCO_MSG(HcoState%Config%Err,MSG )
       WRITE(MSG,*) '   - POG3 fraction           : ', Inst%POG3frac
       CALL HCO_MSG(HcoState%Config%Err,MSG )
       WRITE(MSG,*) '   - POG4 fraction           : ', Inst%POG4frac
       CALL HCO_MSG(HcoState%Config%Err,MSG )
       WRITE(MSG,*) '   - POA5 fraction           : ', Inst%POA5frac
       CALL HCO_MSG(HcoState%Config%Err,MSG )
       !-------------------------------------------------------
       ...
       !-------------------------------------------------------
       !(mrq, 05/20)
       WRITE(MSG,*) '   - NAP fraction            : ', Inst%NAPfrac
       CALL HCO_MSG(HcoState%Config%Err,MSG ) 
       WRITE(MSG,*) '   - IVOC5 fraction          : ', Inst%IVOC5frac
       CALL HCO_MSG(HcoState%Config%Err,MSG )
       WRITE(MSG,*) '   - IVOC4 fraction          : ', Inst%IVOC4frac
       CALL HCO_MSG(HcoState%Config%Err,MSG )       
       !-------------------------------------------------------
    ENDIF
    ...
    DO N = 1, Inst%nSpc
       ...
       IF ( TRIM(SpcName) == 'POG3' ) SpcName = 'OC'   !(mrq, 01/20)
       IF ( TRIM(SpcName) == 'POG4' ) SpcName = 'OC'   !(mrq, 01/20)
       IF ( TRIM(SpcName) == 'POA5' ) SpcName = 'OC'   !(mrq, 06/20)
       !IF ( TRIM(SpcName) == 'NAP'  ) SpcName = 'CO'
       IF ( TRIM(SpcName) == 'NAP'  ) SpcName = 'NAP'  !(mrq, 09/20)
       IF ( TRIM(SpcName) == 'IVOC5') SpcName = 'NAP'  !(mrq, 05/20)
       IF ( TRIM(SpcName) == 'IVOC4') SpcName = 'NAP'  !(mrq, 05/20)
!==============================================================================
! This code is required for partitioning NOx emissions directly to PAN and HNO3.
! We will keep it here as an option for users focusing on North American fires.
! (mps, 5/12/17)
