...
!
  ...
  TYPE :: MyInst
     ...
     INTEGER                        :: IDTHNO2        ! HNO2 tracer ID   !(mrq, 08/20)
...
!
  ...
  !(mrq, 08/20)
  ! The proportions of HONO relative to total NOx emission
  !---------------------------------------
  REAL(hp),  PARAMETER, PRIVATE :: fHONO(NBIOM)  =                 (/ &
        0.00, 0.00, 0.00, 0.92, 0.00, 0.92, 0.55, 0.69, 0.69, 0.28,    &
        0.69, 0.69, 0.28, 0.69, 0.15, 0.10, 0.12, 0.09, 0.09, 0.12,    &
        0.10, 0.75, 0.00, 0.77                                       /)
  !---------------------------------------
...
!
    ...
    REAL(hp), TARGET         :: FLUXHONO_2D(HcoState%NX,HcoState%NY)  !(mrq, 08/20)
    ...
    FLUX_2D = 0e+0_hp
    FLUXHONO_2D = 0e+0_hp   !(mrq, 08/20)

    ! Loop over each land grid-box, removed loop over landpoints
!$OMP PARALLEL DO                                               &
!$OMP DEFAULT( SHARED )                                         &
!$OMP PRIVATE( I, J, DEP_FERT, SOILFRT, FERTDIAG, IJFLUX, IJFLUXHONO )  !(mrq, 08/20) add IJFLUXHONO
    DO J = 1, HcoState%NY
    DO I = 1, HcoState%NX
       ...
       CALL SOIL_NOX_EMISSION( ExtState,  Inst,                &
                               ...
                               Inst%PFACTOR(I,J),   IJFLUX, IJFLUXHONO,  &   !(mrq, 08/20) add IJFLUXHONO
                               DEP_FERT,       FERTDIAG,       &
                               UNITCONV,                       &
                               Inst%CANOPYNOX(I,J,:)                 )
       ...
       FLUXHONO_2D(I,J) = MAX(IJFLUXHONO,0.0_hp)   !(mrq, 08/20) 
...
!$OMP END PARALLEL DO
    ...
    IF ( Inst%SpcScalVal(1) /= 1.0_sp ) THEN
       ...
       FLUXHONO_2D = FLUXHONO_2D * Inst%SpcScalVal(1)   !(mrq, 08/20)
    ENDIF

    ! Eventually apply spatiotemporal scale factors
    CALL HCOX_SCALE ( HcoState, FLUX_2D, TRIM(Inst%SpcScalFldNme(1)), RC )
    CALL HCOX_SCALE ( HcoState, FLUXHONO_2D, TRIM(Inst%SpcScalFldNme(1)), RC )    !(mrq, 12/21)
    IF ( RC /= HCO_SUCCESS ) RETURN
    ...
    !(mrq, 12/21)
    !---------------------------------
    CALL HCO_EmisAdd( HcoState, FLUXHONO_2D, Inst%IDTHNO2, &
                      RC,        ExtNr=Inst%ExtNr )
    IF ( RC /= HCO_SUCCESS ) THEN
...
!
    ...
    IF ( RC /= HCO_SUCCESS ) RETURN
    IF ( nSpc /= 2 ) THEN  ! modify from 1  (mrq, 08/20)
       MSG = 'Module soil NOx accepts only two species!'  !(mrq, 08/20)
       CALL HCO_ERROR(MSG, RC )
       RETURN
    ENDIF
    Inst%IDTNO = HcoIDs(1)
    Inst%IDTHNO2 = HcoIDs(2)   !(mrq, 08/20)
...
!
    ...
    REAL(hp), INTENT(OUT) :: SOILHONO         ! Soil HONO emissions [kg/m2/s]   !(mrq, 08/20)
...
!
    ...
    DO K = 1, 24
       ...
       !(mrq, 08/20)
       !-------------------------------------
       ! determine fSWC value 
       IF ( GWET <= 0.1e+0_hp ) THEN
          fSWC = GWET / 0.1e+0_hp
       ELSEIF (GWET <= 0.4e+0_hp) THEN
          fSWC = ( 0.4e+0_hp - GWET ) / ( 0.4e+0_hp - 0.1e+0_hp )
       ELSE
          fSWC = 0
       ENDIF 
       ! SOILHONO emission
