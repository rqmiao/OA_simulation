...
sitedata = pd.read_csv(input_path + 'OA_research_sites.csv', usecols=['Lon', 'Lat', 'color', 'type'])
nsites = len(sitedata.index)
print('OA site number:', nsites)

# ----------------------------------------------------------------------
# Create a PDF file of the plots
# ----------------------------------------------------------------------
# Define a PDF object so that we can save the plots to PDF
pdf = PdfPages('OA_site_location.pdf')
...
ax.set_extent([73, 135, 18, 50])  #China
...
ax2.add_feature(cfeat.LAND, facecolor='#FFFFF7')    ##FFFFF1
ax2.add_feature(cfeat.OCEAN, facecolor='#D4EBFE')   ##CDE1F3

# ----------------------------------------------------------------------
# Display sites by regions
# ----------------------------------------------------------------------
for indexs in sitedata.index:
    ...
    color = site_coords[2]
    type = 'o'
    ax.plot(lon, lat, type, color=color, transform=ccrs.Geodetic(), ms=12.0, label='OA measurement sites')
    if lat < 22:
        ax2.plot(lon, lat, type, color=color, transform=ccrs.Geodetic(), ms=2.0, label='OA measurement sites')
