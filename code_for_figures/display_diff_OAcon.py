...
def main():
    ...
    # ----------------------------------------------------------------------
    # Path for OA concentrations for the two years
    ...
    for species in range(nspecies):
        ...
        species_con_ref = ds_ref[vname].isel(lev=0)
        species_con_new = ds_new[vname].isel(lev=0)

        OAcon_ref = OAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
        OAcon_new = OAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
        if species_name[species] in POA_species:
            POAcon_ref = POAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
            POAcon_new = POAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
        if species_name[species] in OPOA_species:
            OPOAcon_ref = OPOAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
            OPOAcon_new = OPOAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
        if species_name[species] in ASOA_species:
            ASOAcon_ref = ASOAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
            ASOAcon_new = ASOAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
        if species_name[species] in BSOA_species:
            BSOAcon_ref = BSOAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
            BSOAcon_new = BSOAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
        if species_name[species] in IVSOA_species:
            IVSOAcon_ref = IVSOAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
            IVSOAcon_new = IVSOAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
        if species_name[species] in aqSOA_species:
            aqSOAcon_ref = aqSOAcon_ref + species_con_ref * dataAirDen_ref * MW[species] * ppb_ugm3
            aqSOAcon_new = aqSOAcon_new + species_con_new * dataAirDen_new * MW[species] * ppb_ugm3
    ...
    # ----------------------------------------------------------------------
    # Display the differece between concentrations in the two years
    # ----------------------------------------------------------------------
    extent = [73.75, 135.0, 18.0, 53.5]  # China
    plot = ['POA', 'SOA']
    diffvmax = [12, 5]
    nplot = len(plot)
    unit ='ug/'+u'm\u00B3'
    ...
    for pl in range(len(plot)):
        ...
        gcplot.single_panel(diffcon, vmin=-diffvmax[pl], vmax=diffvmax[pl], use_cmap_RdBu=True,
                            extent=extent, china_map=True, full_china_map=True, coastlines=False, unit=unit)
